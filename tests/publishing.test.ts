...
test('publishing records stay server-only and link analytics are isolated by owner',async()=>{
 const db=new PGlite();try{await db.exec('create role anon; create role authenticated; create role service_role bypassrls; grant usage on schema public to anon,authenticated,service_role;');await db.exec(await readFile(new URL('./fixtures/publishing-schema.sql',import.meta.url),'utf8'));
