...
import {memberRole,teamCookie} from "@/lib/team";
...
export default async function AppPage() {
  ...
  const selected = (await cookies()).get(teamCookie)?.value || userId || "local-demo";
  const role = await memberRole(selected,userId) || "viewer";
  ...
  return (
    <main className="min-h-screen bg-slate-950">
      ...
      {showLibrary ? (
        <><TeamAccess /><Library key={selected} userId={userId || "local-demo"} role={role} /></>
      ) : (
