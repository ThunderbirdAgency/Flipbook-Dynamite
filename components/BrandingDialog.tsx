...
export default function BrandingDialog({ open, onClose, bookId, branding, onChange }: Props) {
  ...
  const [accent, setAccent] = useState(branding.accent || "#50c0e8");
      ...
      <div className="grid h-full grid-rows-[auto_minmax(0,1fr)] md:grid-cols-[360px_minmax(0,1fr)] md:grid-rows-1">
        ...
        <Section label="SEO">
          ...
          <input
            ...
            className="mt-1.5 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white outline-none focus:border-brand-400/60"
          />
          ...
          <textarea
            ...
            className="mt-1.5 w-full resize-none rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white outline-none focus:border-brand-400/60"
          />
        </Section>
        ...
        {[["Midnight", "#101521", "#50c0e8"], ["Studio", "#e8edf2", "#2563eb"], ["Forest", "#10251f", "#6ee7b7"], ["Warm paper", "#eee5d6", "#92400e"]].map(([name, bg, color]) => <button key={name} onClick={() => { setBgColor(bg); setAccent(color); }} className="rounded-lg border border-slate-700 px-3 py-2 text-xs hover:border-brand-400"><span className="mr-2 inline-block h-3 w-3 rounded-full border border-slate-500" style={{backgroundColor:bg}} />{name}</button>)}
        </div>
        ...
        <label className="mb-5 flex items-center gap-3 text-sm"><input type="checkbox" checked={pageSound} onChange={e => setPageSound(e.target.checked)} className="accent-brand-400" />Start with paper sound enabled</label>
        <label className="mb-5 flex items-center gap-3 text-sm"><input type="checkbox" checked={showThumbnails} onChange={e => setShowThumbnails(e.target.checked)} className="accent-brand-400" />Open with page thumbnails visible</label>
...
<label className="flex items-center gap-3 text-sm"><input type="checkbox" checked={extra.showCover!==false} onChange={e=>setExtra({...extra,showCover:e.target.checked})}/>Separate front cover</label><label className="mt-5 block text-sm">Shadow depth<input type="range" min="0" max="1" step="0.1" value={extra.shadow??0.4} onChange={e=>setExtra({...extra,shadow:Number(e.target.value)})} className="mt-3 w-full"/></label><p className="mt-5 text-sm text-slate-400">Responsive page layout adapts to the reader’s screen.</p>
            ...
            <a href={`/book/${bookId}?edit=1`} className="block border-b border-slate-700 px-5 py-5 text-sm font-medium text-brand-300">Add video, links, etc. ↗</a>
          </div>
        </section>
        <aside className="flex min-h-0 min-w-0 flex-col bg-slate-950 p-3 sm:p-5">
          <div className="mb-3 flex shrink-0 items-center justify-between"><h3 className="text-sm text-slate-400">Book preview</h3><a href={`/book/${bookId}`} target="_blank" rel="noopener noreferrer" className="text-xs text-brand-400">Open full size ↗</a></div>
  ...
  return (
    <label className="flex items-center gap-2 text-sm text-slate-300">
      ...
      <input
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="w-28 rounded-lg border border-slate-700 bg-slate-950 px-2 py-1.5 font-mono text-xs text-white outline-none focus:border-brand-400/60"
      />
    </label>
  );
}
