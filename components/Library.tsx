 ...
 return <div className="mx-auto max-w-[1500px] px-4 pb-16 sm:px-8">
  ...
  <div className="grid gap-6 lg:grid-cols-[220px_minmax(0,1fr)]">
   ...
   <section className="min-w-0">
    ...
    {loadError ? <div role="alert" className="rounded-xl border border-red-500/30 p-5 text-sm text-red-300">{loadError}<button onClick={refresh} className="ml-3 underline">Try again</button></div> : !books ? <p className="py-12 text-center text-slate-400">Opening your workspace…</p> : !visible.length ? <div className="rounded-2xl border border-dashed border-slate-700 px-6 py-14 text-center"><h3 className="font-medium text-white">{query ? "No matching flipbooks" : "Room for your next great publication"}</h3><p className="mt-2 text-sm text-slate-500">{query ? "Try another title or clear the search." : "Upload a PDF below, or move an existing flipbook into this folder."}</p></div> : <div className={view === "grid" ? "grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3" : "space-y-3"}>
