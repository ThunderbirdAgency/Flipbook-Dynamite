...
export default function Library({ userId, role }: { userId: string; role: TeamRole }) {
 ...
 return <div className="mx-auto max-w-[1500px] px-4 pb-16 sm:px-8">
  <nav aria-label="Workspace navigation" className="flex flex-wrap gap-2 border-y border-slate-800 py-3">
   {([["books","Flipbooks"],["shelf","Bookshelves"],["custom","Custom links"],["track","Trackable links"]] as const).filter(([id])=>canEdit||id==="books").map(([id,label])=><button key={id} onClick={()=>setSection(id)} aria-current={section===id?"page":undefined} className={`rounded-lg px-5 py-3 text-sm font-medium ${section===id?"bg-amber-400/10 text-amber-300":"text-slate-400 hover:bg-slate-800 hover:text-white"}`}>{label}</button>)}
  ...
  <div className="mb-5 flex flex-wrap items-end justify-between gap-4 border-t border-slate-800 pt-5">
   <div><p className="mb-2 text-xs font-semibold uppercase tracking-[.2em] text-amber-400">Creator workspace</p><h1 className="text-3xl font-semibold tracking-tight text-white">Your flipbooks</h1><p className="mt-2 text-sm text-slate-400">Your publications, clients, and results. All in one place.</p></div>
   ...
   <section className="min-w-0">
    ...
    {loadError ? <div role="alert" className="rounded-xl border border-red-500/30 p-5 text-sm text-red-300">{loadError}<button onClick={refresh} className="ml-3 underline">Try again</button></div> : !books ? <p className="py-12 text-center text-slate-400">Opening your workspace…</p> : !visible.length ? <div className="rounded-2xl border border-dashed border-slate-700 px-6 py-14 text-center"><h3 className="font-medium text-white">{query ? "No matching flipbooks" : "Room for your next great publication"}</h3><p className="mt-2 text-sm text-slate-500">{query ? "Try another title or clear the search." : "Upload a PDF below, or move an existing flipbook into this folder."}</p></div> : <div className={view === "grid" ? "grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3" : "divide-y divide-slate-800 rounded-xl border border-slate-800"}>
     {visible.map(book=><article key={book.id} className={`bg-slate-900/40 p-4 transition hover:bg-slate-800/40 ${view === "list" ? "flex flex-wrap items-center gap-4" : "rounded-xl border border-slate-800"}`}>
      <div className={view === "list" ? "h-20 w-14 shrink-0 overflow-hidden rounded-md bg-slate-800" : "mx-auto mb-4 aspect-[3/4] max-h-52 overflow-hidden rounded-md bg-slate-800"}>{book.status === "ready" ? <Thumbnail id={book.id} /> : <div className="flex h-full items-center justify-center text-xs text-amber-400">PDF</div>}</div>
      <div className={`min-w-0 flex-1 ${view === "list" ? "xl:flex xl:items-center xl:gap-5" : ""}`}><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate font-medium text-white" title={book.title}>{book.status === "ready" ? <Link href={`/book/${book.id}`} className="hover:text-amber-300">{book.title}</Link> : book.title}</h3><p className="mt-1 text-xs text-slate-500">{new Date(book.createdAt).toLocaleDateString()} · {formatSize(book.size)}</p></div><span className={`shrink-0 rounded-md px-2 py-1 text-[10px] ${book.status === "pending" ? "bg-amber-400/10 text-amber-300" : "bg-slate-800 text-slate-300"}`}>{book.status === "pending" ? "Unfinished" : book.hasPassword ? "Password" : book.visibility === "private" ? "Private" : "Shared by link"}</span></div>
    ...
    <div ref={uploadRef} className="mt-8 border-t border-slate-800 pt-6"><h2 className="mb-3 text-lg font-semibold text-white">Add flipbooks</h2><UploadZone onFiles={upload} uploading={uploading} />{stage && <p role="status" className="mt-3 text-sm text-amber-300">{stage}</p>}</div>
   </section>
  </div>
