"use client";
import {permits,type TeamRole} from "@/lib/team-permissions";
...
const field = "w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white outline-none focus:border-brand-400";
...
export default function Library({ userId, role }: { userId: string; role: TeamRole }) {
 const canEdit=permits(role,"edit"), canDelete=permits(role,"delete");
 ...
 const upload = async (files:FileList) => {
  ...
  try {
   ...
   setNotice(`${pdfs.length === 1 ? "Your flipbook is" : "Your flipbooks are"} ready. Open, customize, or share from your library.`);
   if(["pending","private","password","unfiled"].includes(scope)) setScope("all");
   setQuery("");
  } catch(e) { setError((e as Error).message); }
  finally { setUploading(false); setStage(""); await refresh(); }
 };
 const duplicate = async (book: Book) => {
  setBusy(true); setError(""); setNotice(`Copying “${book.title}”…`);
  try {
   const response = await fetch(`/api/books/${book.id}/copy`, { method: "POST" });
   const data = await response.json();
   if (!response.ok) throw new Error(data.error || "Could not copy this flipbook.");
   if (placements[book.id]) await request("/api/workspace", "POST", { action: "move", book: data.book.id, folder: placements[book.id] });
   setQuery(""); setNotice(`“${data.book.title}” is ready, with its own PDF, images, link, and QR code.`);
   await refresh();
  } catch (e) { setNotice(""); setError((e as Error).message); await refresh(); }
  finally { setBusy(false); }
 };
 const copyLink = async (book: Book) => {
  try { await navigator.clipboard.writeText(`${location.origin}/book/${book.id}`); setNotice("Flipbook link copied."); }
  catch { setShareTab("link"); setShare(book); }
 };
 ...
 return <div className="mx-auto max-w-[1500px] px-4 pb-16 sm:px-8">
  <nav aria-label="Workspace navigation" className="flex flex-wrap gap-2 border-y border-slate-800 py-3">
   {([["books","Flipbooks"],["shelf","Bookshelves"],["custom","Custom links"],["track","Trackable links"]] as const).filter(([id])=>canEdit||id==="books").map(([id,label])=><button key={id} onClick={()=>setSection(id)} aria-current={section===id?"page":undefined} className={`rounded-lg px-5 py-3 text-sm font-medium ${section===id?"bg-brand-400/10 text-brand-300":"text-slate-400 hover:bg-slate-800 hover:text-white"}`}>{label}</button>)}
   <QuickStart userId={userId} />
  </nav>
  {!canEdit?<section className="py-8 text-white"><h1 className="text-2xl">Team library · view only</h1>{loadError&&<p role="alert">{loadError}</p>}{books?.map(book=><article key={book.id} className="mt-4 flex flex-wrap gap-4 rounded-xl border border-slate-700 p-5"><h2 className="flex-1">{book.title}</h2><Link href={`/book/${book.id}`}>Open</Link><Link href={`/book/${book.id}/insights`}>Stats</Link><a href={`/api/books/${book.id}/pdf?download=1`}>Download PDF</a></article>)}</section>:section!=="books"?<PublishingPanel key={section} kind={section} books={books||[]}/>:<>
  <div className="mb-5 flex flex-wrap items-end justify-between gap-4 border-t border-slate-800 pt-5">
   <div><p className="mb-2 text-xs font-semibold uppercase tracking-[.2em] text-brand-400">Creator workspace</p><h1 className="text-3xl font-semibold tracking-tight text-white">Your flipbooks</h1><p className="mt-2 text-sm text-slate-400">Your publications, clients, and results. All in one place.</p></div>
   <button onClick={()=>uploadRef.current?.scrollIntoView({behavior:"smooth",block:"center"})} className="rounded-xl bg-brand-400 px-5 py-3 text-sm font-bold text-slate-950 hover:bg-brand-300">＋ Upload PDF</button>
  </div>
  <div className="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
   {[["Published flipbooks",String(all.filter(b=>b.status === "ready").length)],["Total views",String(Object.values(views).reduce((n,v)=>n+v,0))],["Client folders",String(folders.length)],["PDF storage reserved",`${((workspace?.reservedBytes || 0)/(workspace?.storageLimitBytes || 1073741824)*100).toFixed(0)}%`]].map(([label,value])=><div key={label} className="rounded-xl border border-slate-800 bg-slate-900/50 px-4 py-2"><p className="text-xs text-slate-400">{label}</p><p className="mt-1 text-lg font-semibold tabular-nums text-white">{books === null ? "—" : value}</p></div>)}
  </div>
  <div className="grid gap-6 lg:grid-cols-[220px_minmax(0,1fr)]">
   <aside className="self-start rounded-2xl border border-slate-800 bg-slate-900/40 p-3 lg:sticky lg:top-5">
    <nav aria-label="Library filters" className="grid grid-cols-2 gap-1 lg:block">
     {Object.entries(scopeNames).map(([id,label])=><button key={id} onClick={()=>setScope(id)} aria-current={scope === id ? "page" : undefined} className={`flex w-full items-center justify-between rounded-lg px-3 py-2.5 text-left text-sm ${scope === id ? "bg-brand-400/10 font-medium text-brand-300" : "text-slate-400 hover:bg-slate-800 hover:text-white"}`}>{label}{id === "all" && <span className="text-xs">{all.length}</span>}{id === "pending" && <span className="text-xs">{all.filter(b=>b.status === "pending").length}</span>}</button>)}
    </nav>
    <div className="mb-2 mt-6 flex items-center justify-between px-3"><h2 className="text-[11px] font-semibold uppercase tracking-widest text-slate-500">Private folders</h2><button aria-label="Create folder" onClick={()=>openForm({kind:"folder",name:""})} className="px-2 text-xl text-brand-400">＋</button></div>
    <div className="max-h-72 space-y-1 overflow-y-auto">
     {folders.map(f=><button key={f.id} onClick={()=>setScope(f.id)} aria-current={scope === f.id ? "page" : undefined} className={`flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-left text-sm ${scope === f.id ? "bg-brand-400/10 text-brand-300" : "text-slate-300 hover:bg-slate-800"}`}><span aria-hidden="true">▱</span><span className="min-w-0 flex-1 truncate">{f.name}</span><span className="text-xs text-slate-500">{all.filter(b=>placements[b.id] === f.id).length}</span></button>)}
   ...
   <section className="min-w-0">
    {stage && <p role="status" className="mt-3 break-words text-sm text-brand-300">{stage}</p>}
    ...
    <div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div className="min-w-0"><h2 className="break-words text-xl font-semibold text-white">{activeFolder?.name || scopeNames[scope] || "All flipbooks"} <span className="text-sm font-normal text-slate-500">{visible.length}</span></h2>{activeFolder && <div className="mt-2 flex gap-3 text-xs text-slate-400"><button onClick={()=>openForm({kind:"renameFolder",id:activeFolder.id,name:activeFolder.name})}>Rename folder</button><button onClick={()=>openForm({kind:"deleteFolder",id:activeFolder.id,name:activeFolder.name})}>Remove folder</button></div>}</div><button disabled={!visible.length} onClick={exportLibrary} className={button}>Export CSV</button></div>
    <div className="mb-4 flex flex-wrap gap-3"><input aria-label="Search flipbooks" placeholder="Search by title or filename…" value={query} onChange={e=>setQuery(e.target.value)} className={`${field} min-w-48 flex-1`} /><select aria-label="Sort flipbooks" value={sort} onChange={e=>setSort(e.target.value)} className={`${field} sm:!w-auto`}><option value="newest">Newest first</option><option value="oldest">Oldest first</option><option value="name">Title A–Z</option><option value="views">Most viewed</option></select><div className="flex rounded-xl border border-slate-700 p-1">{(["list","grid"] as const).map(v=><button key={v} aria-pressed={view === v} onClick={()=>setView(v)} className={`rounded-lg px-3 text-xs capitalize ${view === v ? "bg-slate-700 text-white" : "text-slate-500"}`}>{v}</button>)}</div></div>
    {loadError ? <div role="alert" className="rounded-xl border border-red-500/30 p-5 text-sm text-red-300">{loadError}<button onClick={refresh} className="ml-3 underline">Try again</button></div> : !books ? <p className="py-12 text-center text-slate-400">Opening your workspace…</p> : !visible.length ? <div className="rounded-2xl border border-dashed border-slate-700 px-6 py-14 text-center"><h3 className="font-medium text-white">{query ? "No matching flipbooks" : "Room for your next great publication"}</h3><p className="mt-2 text-sm text-slate-500">{query ? "Try another title or clear the search." : "Upload a PDF below, or move an existing flipbook into this folder."}</p></div> : <div className={view === "grid" ? "grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3" : "divide-y divide-slate-800 rounded-xl border border-slate-800"}>
     {visible.map(book=><article key={book.id} className={`bg-slate-900/40 p-4 transition hover:bg-slate-800/40 ${view === "list" ? "flex flex-wrap items-center gap-4" : "rounded-xl border border-slate-800"}`}>
      <div className={view === "list" ? "h-20 w-14 shrink-0 overflow-hidden rounded-md bg-slate-800" : "mx-auto mb-4 aspect-[3/4] max-h-52 overflow-hidden rounded-md bg-slate-800"}>{book.status === "ready" ? <Thumbnail id={book.id} /> : <div className="flex h-full items-center justify-center text-xs text-brand-400">PDF</div>}</div>
      <div className={`min-w-0 flex-1 ${view === "list" ? "xl:flex xl:items-center xl:gap-5" : ""}`}><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate font-medium text-white" title={book.title}>{book.status === "ready" ? <Link href={`/book/${book.id}`} className="hover:text-brand-300">{book.title}</Link> : book.title}</h3><p className="mt-1 text-xs text-slate-500">{new Date(book.createdAt).toLocaleDateString()} · {formatSize(book.size)}</p></div><span className={`shrink-0 rounded-md px-2 py-1 text-[10px] ${book.status === "pending" ? "bg-brand-400/10 text-brand-300" : "bg-slate-800 text-slate-300"}`}>{book.status === "pending" ? "Unfinished" : book.hasPassword ? "Password" : book.visibility === "private" ? "Private" : "Shared by link"}</span></div>
      ...
      </div><div className={`flex flex-wrap items-center gap-x-4 gap-y-2 text-sm ${view === "list" ? "mt-4 xl:mt-0 xl:max-w-md xl:justify-end" : "mt-4"}`}>
       {book.status === "ready" ? <><Link href={`/book/${book.id}`} className="font-medium text-brand-400">Open</Link><button onClick={()=>setBranding(book)} className="text-slate-300">Customize</button><Link href={`/book/${book.id}?edit=1`} className="text-slate-300">Editor</Link><button onClick={()=>{setShareTab("link");setShare(book);}} className="text-slate-300">Share</button><button onClick={()=>{setShareTab("embed");setShare(book);}} className="text-slate-300">Embed</button><Link href={`/book/${book.id}/insights`} className="text-slate-300">Stats</Link><details name="book-actions" className="relative"><summary aria-label={`More actions for ${book.title}`} className="cursor-pointer rounded-lg px-2 py-2 text-slate-300 hover:bg-slate-800">More</summary><div className="absolute right-0 z-20 mt-1 w-44 rounded-xl border border-slate-700 bg-slate-900 p-2 shadow-xl"><button onClick={()=>openForm({kind:"renameBook",id:book.id,name:book.title})} className="block w-full rounded-lg px-3 py-2 text-left text-slate-300 hover:bg-slate-800">Rename</button><button disabled={busy} onClick={()=>duplicate(book)} className="block w-full rounded-lg px-3 py-2 text-left text-slate-300 hover:bg-slate-800 disabled:opacity-40">Make a copy</button><button onClick={()=>copyLink(book)} className="block w-full rounded-lg px-3 py-2 text-left text-slate-300 hover:bg-slate-800">Copy link</button><a href={`/api/books/${book.id}/pdf?download=1`} className="block rounded-lg px-3 py-2 text-slate-300 hover:bg-slate-800">Download PDF ↓</a><button disabled={busy || !canDelete} onClick={()=>openForm({kind:"deleteBook",id:book.id,name:book.title})} className="block w-full rounded-lg px-3 py-2 text-left text-red-300 hover:bg-red-500/10">Delete flipbook</button></div></details></> : <><button disabled={busy} onClick={()=>complete(book)} className="rounded-lg border border-brand-400/40 px-3 py-2 text-brand-400 disabled:opacity-60">{checking === book.id ? "Checking PDF…" : "Check upload"}</button>{checking === book.id && <span role="status" className="text-slate-300">Verifying your saved PDF…</span>}</>}
       {book.status === "pending" && <button disabled={busy || !canDelete} onClick={()=>openForm({kind:"deleteBook",id:book.id,name:book.title})} aria-label={`Delete ${book.title}`} className="rounded-lg border border-red-500/30 px-3 py-2 text-red-300 hover:bg-red-500/10">Delete unfinished upload</button>}
      </div></div>
     </article>)}
    </div>}
    <div ref={uploadRef} className="mt-8 border-t border-slate-800 pt-6"><h2 className="mb-3 text-lg font-semibold text-white">Add flipbooks</h2><UploadZone onFiles={upload} uploading={uploading} />{stage && <p role="status" className="mt-3 text-sm text-brand-300">{stage}</p>}</div>
   </section>
  </div>
  {form && <WorkspaceDialog onClose={()=>!busy && setForm(null)} title={form.kind === "folder" ? "Create a folder" : form.kind === "renameFolder" ? "Rename folder" : form.kind === "renameBook" ? "Rename flipbook" : form.kind === "deleteFolder" ? "Remove folder?" : "Delete flipbook?"}><form onSubmit={saveForm}><p className="mb-5 text-sm leading-6 text-slate-400">{form.kind === "deleteFolder" ? `“${form.name}” will be removed. Its flipbooks will stay in your library, unfiled.` : form.kind === "deleteBook" ? `“${form.name}” and its files will be removed. This cannot be undone.` : "Use a name you’ll recognize when your library grows."}</p>{!form.kind.startsWith("delete") && <label className="text-xs text-slate-400">Name<input autoFocus required maxLength={form.kind === "renameBook" ? 200 : 80} value={formValue} onChange={e=>setFormValue(e.target.value)} className={`${field} mt-2`} /></label>}{formError && <p role="alert" className="mt-3 text-sm text-red-300">{formError}</p>}<div className="mt-6 flex justify-end gap-3"><button type="button" disabled={busy} onClick={()=>setForm(null)} className={button}>Cancel</button><button disabled={busy || (!form.kind.startsWith("delete") && !formValue.trim())} className={`rounded-lg px-4 py-2 text-sm font-semibold disabled:opacity-40 ${form.kind.startsWith("delete") ? "bg-red-500 text-white" : "bg-brand-400 text-slate-950"}`}>{busy ? "Saving…" : form.kind.startsWith("delete") ? "Remove" : "Save"}</button></div></form></WorkspaceDialog>}
  </>}
  {share && <ShareDialog key={`${share.id}-${shareTab}`} initialTab={shareTab} open onClose={()=>setShare(null)} title={share.title} bookId={share.id} isOwner shareUrl={`${window.location.origin}/book/${share.id}`} embedUrl={`${window.location.origin}/embed/${share.id}`} visibility={share.visibility} hasPassword={share.hasPassword} onPrivacyChange={(visibility,hasPassword)=>{setShare({...share,visibility,hasPassword}); void refresh();}} />}
  ...
  return (
    ...
    >
      ...
      {uploading ? (
        ...
        <>
          <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-brand-400/10 text-brand-400 transition group-hover:scale-105">
          ...
          <p className="text-base font-medium text-white">
            Drop a PDF here, or <span className="text-brand-400">browse</span>
          </p>
